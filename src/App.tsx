...
/* ===================================================================================
   Sub-components 子元件定義 (確保在 App 之前載入)
=================================================================================== */
...
const ContextualMenuItem = ({ item, isActive, onClick, onSave, onDelete, isEditMode }) => {
  ...
  return (
    <div className={`relative flex items-center justify-between px-4 py-3.5 rounded-2xl cursor-pointer transition-all w-full group ${isActive ? 'bg-[#111111] text-white shadow-lg font-bold' : 'text-gray-600 hover:bg-white/80 font-bold'}`} onClick={onClick}>
        ...
        {[
          ...
          <div key={i} className="bg-white p-5 rounded-[1.5rem] shadow-sm border border-gray-100 flex flex-col relative overflow-hidden">
             ...
             <div className="flex items-baseline gap-1">
               ...
               <span className="text-sm font-bold text-gray-400 mb-1">公斤</span>
             </div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white rounded-[1.5rem] p-5 shadow-sm border border-gray-100 mb-6 shrink-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
          ...
          <div className="w-full md:col-span-2 flex gap-3">
             ...
             <div className="w-24">
               <label className="block text-xs font-bold text-gray-700 mb-2">數量 (公斤)</label>
        ...
        <table className="w-full text-left border-collapse min-w-[700px]">
          <thead>
            <tr className="bg-gray-50 text-gray-500 text-sm border-b border-gray-100">
              ...
              <th className="p-4 md:p-5 font-bold text-right whitespace-nowrap">數量 (公斤)</th>
...
export default function App() {
  ...
  const [staffMembers, setStaffMembers, isL10] = useFirestoreState('workOS_v1', 'staffMembers', [
    { id: 's1', name: '王大明' }, { id: 's2', name: '林小美' }, { id: 's3', name: '陳建國' }
  ]);
  ...
  const getMenuProps = (menuId) => {
    ...
    if (menuId === 'workflow') { Icon = Briefcase; }
              ...
              <div className="animate-fade-in max-w-2xl">
                ...
                <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 mb-6">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                    <input type="text" placeholder="輸入人員姓名..." value={newStaffName} onChange={e => setNewStaffName(e.target.value)} onKeyDown={e => e.key==='Enter' && newStaffName.trim() && (() => { setStaffMembers([...staffMembers, {id: 's'+Date.now(), name: newStaffName.trim()}]); setNewStaffName(''); })()} className="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm font-bold outline-none focus:border-black focus:ring-1 focus:ring-black" />
                    <button onClick={() => { if(newStaffName.trim()) { setStaffMembers([...staffMembers, {id: 's'+Date.now(), name: newStaffName.trim()}]); setNewStaffName(''); } }} className="bg-black text-white px-6 py-2.5 rounded-xl font-bold hover:bg-gray-800 transition-colors">新增</button>
                  </div>
                </div>
